...
def _dense_to_qlora(module):
  if isinstance(module, nn.Dense):
    return peft.QLoRADense(
        rank=1, 
        wrapped=module,
        quant_method=peft.QuantizationMethod.INT4
    )
...
def test_qlora():
  model = MyModule()
  with peft.ModuleInterceptor(_dense_to_qlora):
    out, params = model.init_with_output(
        jax.random.key(0),
        jnp.zeros((1, 4)),
    )

  # Structure should contain both the parameters for the quantized model
