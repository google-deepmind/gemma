...
from gemma.gm.nn.nano import _nano as nano_models
from gemma.gm.nn.nano import _transformer as nt
import jax
...
def _get_output(model: nt.Transformer, **kwargs) -> tuple[gm.nn.Output, Any]:
...
def test_transformer(model_cls: type[nt.Transformer]):
...
def test_images():
  ...
  model = nano_models.Nano3()  # pylint: disable=missing-kwoa  # pytype: disable=missing-parameter
...
def test_text_only():
  ...
  model = nano_models.Nano3(text_only=True)
...
def test_last_only():
  model = nano_models.Nano3(return_last_only=True)
  tokens = jnp.ones((BATCH_SIZE, SEQ_LEN), dtype=jnp.int32)
  out, params = _get_output(model, tokens=tokens)
